...
for config in RUN_CONFIGS:
    ...
    if config["type"] == "emg":  # Adjust path logic for EMG
        pbt_home = path.join(run_home, f"pbt_runs/{config['type']}/")
        model_dir = path.join(pbt_home, "run_001/pbt_run/best_model")
    else:  # Keep the existing logic for spikes
...
for session_id in session_ids:
    ...
    for config in RUN_CONFIGS:
        ...
        interface_pattern = f"*{session_id}*interface.pkl" if is_emg else f"{ds_name}_*_spikes_10_interface.pkl"


        # Debug: Print the directory and pattern being searched
        print(f"Searching for interface file in: {lfads_input_dir}")
        print(f"Using pattern: {interface_pattern}")

        try:
            int_path = glob.glob(os.path.join(lfads_input_dir, interface_pattern))[0]
            print(f"Found interface file for {run_name}: {int_path}")
            with open(int_path, "rb") as f:
                interface = pickle.load(f)
        except IndexError:
            logger.warning(f"   Skipping {run_name}: Interface not found.")
            print(f"Interface file not found. Searched in: {lfads_input_dir}")
            print(f"Pattern used: {interface_pattern}")
            continue
        ...
        # if is_emg:
        #     # 1. Names for the dataset levels
        #     names = ["deEMG_mean", "deEMG_var", "deEMG_factors", "deEMG_gen_inputs", "lfads_factors_ALL", "lfads_rates_ALL"]
            
        #     # 2. Update the merge map
        #     interface.merge_fields_map = {
        #         "output_par_1": names[0],
        #         "output_par_2": names[1],
        #         "factors": names[2],
        #         "gen_inputs": names[3],
        #         "factors_ALL": names[4],
        #         "rates_ALL": names[5]
        ...
        #     # 3. Handle the data stacking
        #     data_dict["output_par_1"] = np.squeeze(sampling_output[lfads_key].output_params[:, :, :, 0])
        #     data_dict["output_par_2"] = np.squeeze(sampling_output[lfads_key].output_params[:, :, :, 1])
            
        #     # Use hstack to bilateralize
        #     logger.debug(f"Attributes of sampling_output[{lfads_key}]: {dir(sampling_output[lfads_key])}")
        #     data_dict["lfads_factors_ALL"] = np.hstack([sampling_output[lfads_key].factors_L, 
        #                                         sampling_output[lfads_key].factors_R])
            
        #     # Combine the rates
        #     stacked_rates = np.hstack([sampling_output[lfads_key].rates_L, 
        #                                sampling_output[lfads_key].rates_R])
        #     data_dict["rates_ALL"] = stacked_rates

        #     # IMPORTANT: Manually add it if the interface merge skips it
        #     safe_add_data(dataset, data_dict["rates_ALL"], f"lfads_rates_ALL{suffix}")
        #     safe_add_data(dataset, data_dict["factors_ALL"], f"lfads_factors_ALL{suffix}")
...
# Save all_ds to a file
output_file = os.path.join(input_dir, "all_ds.pkl")

with open(output_file, "wb") as f:
    pickle.dump(all_ds, f, protocol=4)

print(f"Saved all_ds to {output_file}")
# %%
