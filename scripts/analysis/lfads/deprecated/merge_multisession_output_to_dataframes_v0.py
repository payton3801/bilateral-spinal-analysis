# %%
########################
# blending windows to create a continuous df
# restores timestamps and overlaps
#########################
# # import rds
import sys
import pdb
...
BASE_NAME = "binsize_10ms_pcr_ALL"
use_cached = True
...
lfads_dataset_dir_2 = path.join(RUN_HOME, "datasets")
...
for session_id in session_ids:
    ...
    interface_name = f"pkls/{ds_name}_emg_10_interface.pkl"

    #interface_name = f"pkls/{ds_name}*interface.pkl"
    #interface_name = f"{ds_name}_emg_10_interface.pkl"    
    #    interface_name = f"{ds_name}_{ARRAY_SELECT}_spikes_10_interface.pkl"    

    h5_name = f"*{ds_name}*.h5"
    cfg_yaml_filepath = glob.glob(os.path.join(lfads_dataset_dir, yaml_name))[0]
    interface_filepath = glob.glob(os.path.join(lfads_dataset_dir_2, interface_name))[0]
   ...
   # -- merge lfads output to continuous-like form
    ...
    valid_unit_indices = dataset.data[DATA_FIELDNAME].columns.values
    ...
    if ARRAY_SELECT == "L":
        ...
        side_suffix = "_L"  # e.g., "lfads_factors_L"
    elif ARRAY_SELECT == "R":
        candidate_indices = np.where([("R side" in loc) for loc in all_locations])[0]
        side_suffix = "_R"  # e.g., "lfads_factors_R"
    elif ARRAY_SELECT == "ALL":
        candidate_indices = np.where(
            [("L side" in loc or "R side" in loc) for loc in all_locations]
        )[0]
        side_suffix = "_ALL"  # e.g., "lfads_factors_ALL"
    else:
        raise ValueError("ARRAY_SELECT must be 'L', 'R', or 'ALL'")

    # 2. SORT CANDIDATES BY ACTIVITY
    raw_spike_counts = []
    ...
    for MO_FIELD, MATCH_FIELDNAME in zip(MO_FIELDS, MATCH_FIELDNAMES):
        ...
        save_name = MO_FIELD + side_suffix 
        ...
        if MATCH_FIELDNAME:
            ...
            if num_data_cols < len(sorted_candidates):
                ...
                print(f"  -> {MO_FIELD}: Dropped quiet units. Matching {num_data_cols} cols to: {chan_names}")
            ...
            # Final fail-safe slice
            # min_len = min(data_to_add.shape[1], len(chan_names))
            # data_to_add = data_to_add[:, :min_len]
            # chan_names = chan_names[:min_len]
        ...
        # Check against 'save_name' (e.g. lfads_factors_L), NOT MO_FIELD
        if save_name in dataset.data.columns.levels[0]:
            logger.warning(f"Overwriting {save_name} in dataframe")
            dataset.data = dataset.data.drop(
                [save_name], level=0, axis=1
            ) 
        else:
            logger.info(f"Adding {save_name} in dataframe")

        # Add the data with the side-specific name
        dataset.add_continuous_data(
            data_to_add, save_name, chan_names=chan_names
        )

        # drop duplicated columns
    dataset.data = dataset.data.loc[:, ~dataset.data.columns.duplicated()]
    ds_savepath = path.join(nwb_cache_dir, f"nlb_{DATASET_NAME}_{session_id}.pkl")
