...
output_dir = path.join(nwb_cache_dir, "merged_datasets") # Save individual files here
...
def apply_step_info(dataset, ds_name):
    ...
    if not path.exists(info_path):
        logger.warning(f"Step info NOT found for {ds_name}")
        return

    with open(info_path, "rb") as f:
        step_data = pickle.load(f)

    # Attach the tables to the dataset object
    ...
    logger.info(f"   -> Attached Step Info tables.")

def safe_add_data(dataset, data_block, save_name, chan_names=None):
    """Safely adds continuous data to the dataset, overwriting if needed."""
...
for session_id in session_ids:
    ...
    if path.exists(ds_path):
        ...
        logger.info("   Cache not found, loading from NWB...")
    ...
    for config in RUN_CONFIGS:
        ...
        # 1. Find the LFADS key for this session
        ...
        try:
            ...
            logger.warning(f"   Skipping {run_name}: Input H5 not found for session.")
            continue

        if lfads_key not in sampling_output:
            logger.warning(f"   Skipping {run_name}: Key {lfads_key} not in model output.")
            continue
    ...
    logger.info(f"✅ Saved merged dataset to: {final_save_path}")

print("\nAll sessions processed and saved individually.")
...
if 'lfads_rates_R.0' in ds.data.columns:
    display(ds.data['lfads_rates_R.0'].head())
# %%
