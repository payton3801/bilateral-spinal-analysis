...
for session_id in session_ids:
    ...
    if ARRAY_SELECT == "L":
        ...
        spk_keep_mask = np.ones(len(dataset.unit_info['location']), dtype=bool) 


    # -- xcorr rejection
    pair_xcorr, chan_names_to_drop = dataset.get_pair_xcorr(
        "emg", threshold=XCORR_THRESHOLD, zero_chans=True
    )

    # -- update keep mask
    #emg_keep_mask[chan_names_to_drop] = False
    chan_indices_to_drop = [
        i for i, name in enumerate(dataset.unit_info['location']) if name in chan_names_to_drop
    ]

# Update spk_keep_mask using the indices
    spk_keep_mask[chan_indices_to_drop] = False


    #import pdb; pdb.set_trace()


    spk_names = dataset.data['spikes'].columns.values  # Access the 'spikes' signal type    # drop_spk_names = spk_names[~spk_keep_mask]
    keep_spk_names = spk_names[spk_keep_mask]

    # Align emg_keep_mask with the size of emg_names
    emg_names = dataset.data.emg.columns.values
    emg_keep_mask = np.ones(len(emg_names), dtype=bool)  # Keep all EMG channels

    # Get the channels to drop and keep
    drop_emg_names = emg_names[~emg_keep_mask]
    keep_emg_names = emg_names[emg_keep_mask]
    ...
    # Drop spike channels based on spk_keep_mask
    logger.info(f"Keep spike channels: {np.sum(spk_keep_mask)}/{spk_keep_mask.size}")
    if np.any(~spk_keep_mask):  # Check if there are channels to drop
        spk_names = dataset.data.spikes.columns.values  # Adjust 'spikes' to the correct field name
    ...
    # logger.info(f"Keep EMG channels: {np.sum(emg_keep_mask)}/{emg_keep_mask.size}")
    # if len(drop_emg_names) > 0:
    #     dataset.data.drop(
    #         columns=drop_emg_names.tolist(), axis=1, level=1, inplace=True
    # )
    logger.info(f"Keep EMG channels: {np.sum(emg_keep_mask)}/{emg_keep_mask.size}")
    if np.any(~emg_keep_mask):  # Check if there are channels to drop
        drop_emg_names = emg_names[~emg_keep_mask]
        dataset.data.drop(
            columns=drop_emg_names.tolist(), axis=1, level=1, inplace=True
        )
    ...
    # -- smooth spikes
    dataset.smooth_spk(
        GAUSS_WIDTH_MS, signal_type="emg", name=f"smooth_{GAUSS_WIDTH_MS}ms"
    )
    smooth_emg_field = f"emg_smooth_{GAUSS_WIDTH_MS}ms"
    ...
    for clip_q_cutoff, emg_name in zip(clip_q, emg_names):
        ...
        print(f"Channel: {emg_name}, Scale Quantile: {scale_q}")
        
        # --- FIX: Prevent Division by Zero ---
        if scale_q <= 0:
            logger.warning(f"Scale quantile for {emg_name} is 0. Setting data to 0 to avoid NaNs.")
            scale_emg = clip_chan_emg * 0.0  # Keep it as zeros
        else:
            scale_emg = clip_chan_emg / scale_q
        # -------------------------------------

        print(f"Scaled Data for {emg_name}: {scale_emg.describe()}")
    ...
    logger.info("Re-rectifying data (clipping < 0 to 0) to prevent NaNs.")
    dataset.data = dataset.data.clip(lower=0.0)
    ...
    axs[0].pcolor(dataset.data.emg_smooth_30ms.values.T); 
    ...
    if len(ti) > 10:
        ...
        # For short datasets, don't drop anything to avoid "Empty DataFrame"
        logger.warning(f"Short dataset detected ({len(ti)} trials). Skipping trial exclusion.")
        excluded_trials = []
    ...
    if dw._t_df.trial_id.nunique() == 0:
        logger.warning(f"No valid trials for session {session_id}. Skipping.")
        continue
...
if debug:
    ...
    ax.set_zlabel("PC3")
    ax2.set_zlabel("PC3")
    ax.set_title(f"{ARRAY_SELECT} emg")
    ax2.set_title("EMG")
    plt.suptitle("Multi-session PCR")

    plt.figure()
    plt.plot(
        np.cumsum(pca_spk.explained_variance_ratio_), "-o", color="g", label="emg"
    )
