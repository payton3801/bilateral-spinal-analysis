...
def load_data(data_dir, 
    ...
    for h5_filename in h5_filenames:
        ...
        assert 'train_' + signal in h5dict, \
        ...
        if merge_tv:
            ...
            if 'train_inds' in h5dict and 'valid_inds' in h5dict:
                ...
                logger.info("No indices found for merge. "
...
def chop_data(data, overlap, window):
    ...
    chopped = np.lib.stride_tricks.as_strided(
        data, shape=shape, strides=strides).copy().astype('f')
    return chopped
...
def load_posterior_averages(model_dir,
    ...
    with h5py.File(h5_filename, 'r') as h5file:
        ...
        datasets = np.unique([str('_').join(x.split('_')[2:]) for x in h5datasets])
    ...
    for ds in h5dict.keys():
        output = {}
        train_inds = h5dict[ds]['train_inds'].astype('int').squeeze()
        valid_inds = h5dict[ds]['valid_inds'].astype('int').squeeze()
