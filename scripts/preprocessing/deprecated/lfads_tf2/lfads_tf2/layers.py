...
from tensorflow.keras.layers import Dropout, Layer, Dense, SimpleRNNCell, GRUCell, RNN, Bidirectional
from tensorflow.keras import Model, Sequential
import tensorflow_probability as tfp
tfd = tfp.distributions
tfb = tfp.bijectors

from lfads_tf2.regularizers import DynamicL2, DynamicL1
from lfads_tf2 import regularizers
from lfads_tf2.tuples import EncoderOutput, DecoderInput, \
    DecoderRNNInput, DecoderState, DecoderOutput, LFADSInput
...
from lfads_tf2 import recon_losses

class BaseMultiDatasetDense(Layer):
    """
    Defines a base linear transformation (Dense) layer that handles
    trainsforming data from multiple sessions. Each session has a unique trainable
    linear transformation that maps the data into the model. Note that `BaseMultiDatasetDense` 
    is a subclass of `tensorflow.keras.Layer`.
    """
    def __init__(self, cfg_node, ds_names, ds_dims, ds_idx, input_dim, output_dim):
        """ Initializes an `ReadIn` object. 
        ...
        ds_names : list
            Filenames for each dataset. Used as dict keys for ds_dims and ds_idx.
        ds_dims : dict
            Dimensionality for each dataset. Indexed using ds_names.
        ds_idx : dict
            Integer number assigned to each dataset. Indexed using ds_names.
            Used in call function to handle passing data through appropriate
            linear transformation.
        """
        super(BaseMultiDatasetDense, self).__init__()
        # store the config
        self.cfg_node = cfg_node

        # ds names, dims, and idx will be used for organizing layers
        # and accurately transform shuffled batch data
        ...
        # for each dataset, create a new dense layer
        # store in dict indexed with dataset names
        self.dense_linear = {}
        for ds_name in self.ds_names:
            self.dense_linear[ds_name] = Sequential([
        ...
        data_shape = [None, mcfg.SEQ_LEN, self.input_dim]
        ds_ixs_shape = [None]
        self.graph_call = tf.function(
            func=self.call,
            input_signature=[
                tf.TensorSpec(shape=data_shape),
                tf.TensorSpec(shape=ds_ixs_shape)])
    ...
    def build(self, input_shape):
        ...
        """
        super(BaseMultiDatasetDense, self).build(input_shape)

    def call(self, data, batch_ds_ixs):
        """Passes shuffled batch data through appropriate linear
        transformation for each dataset.
        ...
        input_shape : tf.TensorShape
            The shape of the inputs to the encoder.
        batch_ds_ixs : tf.Tensor
        """

        mcfg = self.cfg_node.MODEL

        output = tf.fill(
            (tf.shape(data)[0], mcfg.SEQ_LEN, self.output_dim), np.nan
        )
        # split data into
        num_partitions = len(self.ds_names)
        # splits batch data into list (len=num_partitions) separating data by dataset
        split_ds_data = tf.dynamic_partition(data, tf.cast(batch_ds_ixs, tf.int32), num_partitions)
        # splits batch array ixs into list (len=num_partitions) separating ixs by dataset
        split_batch_ixs = tf.dynamic_partition(tf.range(tf.shape(data)[0]), tf.cast(batch_ds_ixs, tf.int32), num_partitions)
        split_output = []
        for ds_data, ds_name in zip(split_ds_data, self.ds_names):
            #ds_dim = self.ds_dims[ds_name]
            #split_output.append(self.dense_linear[ds_name](ds_data[:,:,:ds_dim]))
            split_output.append(self.dense_linear[ds_name](ds_data))
            
        output = tf.dynamic_stitch(split_batch_ixs, split_output)

        return output

    def freeze_matrices(self):
        mcfg = self.cfg_node.MODEL

        batch_size = len(self.ds_names)
        
        data_shape = [batch_size, mcfg.SEQ_LEN, self.input_dim]

        ds_id_ixs = np.array([self.ds_idx[ds] for ds in np.array(self.ds_names)])
        noise = np.ones(shape=data_shape, dtype=np.float32)
        self.call(noise, ds_id_ixs)
        for k in self.dense_linear.keys(): 
            for var in self.dense_linear[k].trainable_variables: 
                var._trainable = False
        
    def get_config(self):
        """ Get the configuration for the ReadIn.
    ...
    def from_config(cls, config):
        """ Initialize an ReadIn from this config.
...
class RegularizedMultiDatasetDense(BaseMultiDatasetDense):
    def __init__(self, *args,
                 reg_scale=0.0, reg_type='DynamicL2'):
        super(RegularizedMultiDatasetDense, self).__init__(*args)
        ...
        for ds_name in self.ds_names:
            self.dense_linear[ds_name] = Sequential([
                Dense(self.output_dim,
                      kernel_initializer=variance_scaling,
                      kernel_regularizer= getattr(regularizers, self.reg_type)(scale=self.reg_scale),
                      bias_initializer=variance_scaling,
                      name='dense_linear'),
            ])
    def update_config(self, config, reg_scale):
        ...
        """
        self.reg_scale = reg_scale
        self.cfg_node = config['cfg_node']
        readin_update = {
            'scale': reg_scale}
        for k in self.dense_linear.keys():
            self.dense_linear[k].layers[0].kernel_regularizer.update_config(readin_update)
    
class ModelReadin(RegularizedMultiDatasetDense):
    def __init__(self, *args, pre_init_from_file=False, **kwargs):
        super(ModelReadin, self).__init__(*args)
        self.readin_dim = self.output_dim
        if pre_init_from_file:
            filepath = os.path.join(self.cfg_node.TRAIN.DATA.DIR, 'pcr_alignment.h5')
            # open alignment matrices from file
            alignment_matrices = {}
            alignment_biases = {}
            with h5py.File(filepath, 'r') as pcr_file:
                for ds_name in self.ds_names:
                    alignment_matrices[ds_name] = pcr_file[ds_name]['matrix'][()]
                    alignment_biases[ds_name] = pcr_file[ds_name]['bias'][()] 

            for ds_name in self.ds_names:
                # fill nan dims with 0's for alignment matrices, then initialize  layer
                ds_dim = self.ds_dims[ds_name]
                dim_diff = self.cfg_node.MODEL.DATA_DIM - ds_dim
                alignment_matrices[ds_name] = np.pad(alignment_matrices[ds_name],
                                               ((0,dim_diff),(0,0)),
                                               constant_values=(0))
                assert self.cfg_node.MODEL.READIN_DIM == alignment_matrices[ds_name].shape[1], \
                    (
                        f"dim of align matrix for {ds_name} ({alignment_matrices[ds_name].shape[1]})"
                        f"does not match cfg.MODEL.READIN_DIM ({readin_dim})" ######added self here to try something
                    )
                self.dense_linear[ds_name] = Sequential([
                    Dense(self.readin_dim,
                          kernel_initializer=PCRInitializer(
                              alignment_matrices[ds_name], matrix_type='weight'
                          ),
                          kernel_regularizer=getattr(regularizers,self.reg_type)(
                              scale= self.reg_scale
                          ),
                          bias_initializer=PCRInitializer(
                              alignment_biases[ds_name], matrix_type='bias'
                          ),
                          name='dense_linear'),
                ])

class ModelReadout(BaseMultiDatasetDense):
    def __init__(self, *args, pre_init_from_file=False):
        super(ModelReadout, self).__init__(*args)
        self.recon_loss = getattr(recon_losses, self.cfg_node.MODEL.RECON_TYPE)()
        n_outparams = self.recon_loss.n_params
        """
        if pre_init_from_file:
            filepath = os.path.join(self.cfg_node.TRAIN.DATA.DIR, 'pcr_alignment.h5')
            # open alignment matrices from file
            ...
            with h5py.File(filepath, 'r') as pcr_file:
                for ds_name in self.ds_names:
                    alignment_matrices[ds_name] = pcr_file[ds_name]['matrix'][()]
                    ds_dim = self.ds_dims[ds_name]
                    dim_diff = self.output_dim - ds_dim
                    alignment_matrices[ds_name] = np.pad(alignment_matrices[ds_name],
                                                         ((0,dim_diff),(0,0)),
                                                         constant_values=(0))
            # choose initializer
            if pre_init_from_file and n_outparams == 1:
                kernel_init=PCRInitializer(
                    alignment_matrices[ds_name].T, matrix_type='weight'
                )
            else:
                kernel_init=variance_scaling
        """
        # create readout layer for each dataset
        for ds_name in self.ds_names:
            self.dense_linear[ds_name] = Sequential([                    
                Dense(self.output_dim*n_outparams,                          
                      kernel_initializer=variance_scaling,
                      name='dense_linear')
            ])                                   
...
class Encoder(Layer):
    ...
    def __init__(self, cfg_node):
        ...
        if self.use_con:
            ...
            ci_enc_cell = ClippedGRUCell(
                ci_enc_dim,
                kernel_initializer=make_variance_scaling(ci_enc_dim + mcfg.ENC_INPUT_DIM),
                recurrent_initializer=make_variance_scaling(ci_enc_dim + mcfg.ENC_INPUT_DIM),
        ...
        ic_enc_cell = ClippedGRUCell(
            ic_enc_dim,
            kernel_initializer=make_variance_scaling(ic_enc_dim + mcfg.ENC_INPUT_DIM),
            recurrent_initializer=make_variance_scaling(ic_enc_dim + mcfg.ENC_INPUT_DIM),
        ...
        # ===== AUTOGRAPH FUNCTIONS =====
        data_shape = [None, mcfg.SEQ_LEN, mcfg.ENC_INPUT_DIM]
    ...
    def call(self, data, training=False):
        ...
        seq_len = self.cfg_node.MODEL.SEQ_LEN
        ...
        h_n = self.ic_enc_bigru(data, initial_state=ic_enc_h0)
        h_n_drop = self.dropout(h_n, training)
        ic_mean = self.ic_mean_linear(h_n_drop)
        ic_logvar = self.ic_logvar_linear(h_n_drop)
        ic_stddev = tf.sqrt(tf.exp(ic_logvar) + self.cfg_node.MODEL.IC_POST_VAR_MIN)

        if self.cfg_node.MODEL.CI_ENC_DIM > 0:
            # tile the initial states so they look like the data and unstack fw and bw
            ci_enc_h0 = tf.unstack(tf.tile(self.ci_enc_h0, [len(data), 1, 1]), axis=1)
            # compute encodings for the controller
            ci_fwd, ci_bwd = self.ci_enc_bigru(data, initial_state=ci_enc_h0)
            # add a lag to the controller input
            ci_fwd = tf.pad(ci_fwd, [[0, 0], [self.cfg_node.MODEL.CI_LAG, 0], [0, 0]])
            ci_bwd = tf.pad(ci_bwd, [[0, 0], [0, self.cfg_node.MODEL.CI_LAG], [0, 0]])
            ...
            ci = tf.zeros_like(data)[:, :, :2*self.cfg_node.MODEL.CI_ENC_DIM]
...
class Decoder(Layer):
    ...
    def __init__(self, cfg_node):
        ...
        self.dropout_rate = tf.Variable(
            cfg_node.MODEL.DROPOUT_RATE, trainable=False)
        self.dropout = Dropout(self.dropout_rate)

        self.use_con = all([
            cfg_node.MODEL.CI_ENC_DIM > 0,
            cfg_node.MODEL.CON_DIM > 0,
            cfg_node.MODEL.CO_DIM > 0,
        ])
        # create the linear mapping from ICs to gen_state
        self.ic_to_g0 = Dense(
            cfg_node.MODEL.GEN_DIM, 
            kernel_initializer=variance_scaling, 
            name='ic_to_g0')
        ...
        # ===== AUTOGRAPH FUNCTIONS =====
        ic_shape = [None, self.cfg_node.MODEL.IC_DIM]
        ci_shape = [
            None, 
            self.cfg_node.MODEL.SEQ_LEN, 
            self.cfg_node.MODEL.CI_ENC_DIM * 2]
        co_shape =[
            None, 
            self.cfg_node.MODEL.SEQ_LEN, 
            self.cfg_node.MODEL.CO_DIM]
        ext_input_shape = [
            None, 
            self.cfg_node.MODEL.SEQ_LEN, 
            self.cfg_node.MODEL.EXT_INPUT_DIM]
        if self.cfg_node.MODEL.EXT_INPUT_READIN_DIM > 0:
            ext_input_shape[2] = self.cfg_node.MODEL.EXT_INPUT_READIN_DIM
        self.graph_call = tf.function(
            func=self.call,
            input_signature=[
                DecoderInput(
    ...
    def call(self, dec_input, training=False):
...
class ClippedSimpleCell(SimpleRNNCell):
    """ Adds hidden state clipping, trainable initial states, and dynamic 
    L2 penalty to the SimpleRNNCell implementation. Refer to tf.keras.SimpleRNNCell 
    docs for args. 
    """
    def __init__(self,
                 *args,
                 ...
                 train_h0=False,
                 **kwargs):
        """Creates a SimpleRNNCell (vanilla) with hidden state clipping and 
        trainable hidden states.
        ...
        clip_value : float, optional
            The value at which to clip the hidden states of the 
            SimpleRNNCell, by default np.inf
        train_h0 : bool, optional
            Whether to use a trainable initial state, by default False
        """
        ...
        self.clip_value = tf.Variable(clip_value, trainable=False)
        self.train_h0 = train_h0
        # the SimpleRNNCell class applies the regularization
        super(ClippedSimpleCell, self).__init__(*args, **kwargs)
        # TODO: find a better place to put this, where scope is perserved... 
        # I wanted to put it in the build function, but there appear to be 
        # cases in the bidirectional wrapper where get_initial_state is 
        # called before build.
    ...
    def call(self, *args, **kwargs):
        """Performs one forward pass of the SimpleRNNCell.

        This function is a wrapper around tf.keras.layers.SimpleRNNCell.call
        that clips the value of the hidden state after every forward 
        pass through the cell. Arguments are identical to the superclass.
        """
        h, _ = super(ClippedSimpleCell, self).call(*args, **kwargs)
        h = tf.clip_by_value(h, -self.clip_value, self.clip_value)
        return h, [h]

    def get_initial_state(self, **kwargs):
        """Returns the initial state of the SimpleRNNCell.

        A wrapper around tf.keras.layers.SimpleRNNCell.get_initial_state
        that returns the trainable initial state if requested.
        """
        if self.train_h0:
            return tf.tile(self.cell_h0, [kwargs['batch_size'], 1])
        else:
            return super(ClippedSimpleCell, self).get_initial_state(**kwargs)

    def get_config(self):
        """ Get the configuration for the ClippedSimpleCell.
    ...
    @classmethod
    def from_config(cls, config):
        """ Initialize a ClippedSimpleCell from this config.
        ...
        Returns
        -------
        lfads_tf2.layers.ClippedSimpleCell
            A ClippedSimpleCell from this config node.
        """
    ...
    def update_config(self, config):
        """Updates the configuration of the ClippedSimpleCell.
...
class DecoderCell(Layer):
    ...
    def __init__(self, cfg_node, **kwargs):
        ...
        # create the dropout layer
        self.dropout_rate = tf.Variable(cfg_node.MODEL.DROPOUT_RATE, trainable=False)
        self.dropout = Dropout(self.dropout_rate)

        # collect the appropriate dimensions from cfg_node
        self.ci_enc_dim = ci_enc_dim = cfg_node.MODEL.CI_ENC_DIM
        self.gen_units = gen_units = cfg_node.MODEL.GEN_DIM
        self.con_units = con_units = cfg_node.MODEL.CON_DIM
        self.fac_units = fac_units = cfg_node.MODEL.FAC_DIM
        self.co_units = co_units = cfg_node.MODEL.CO_DIM
        self.ext_units = ext_units = cfg_node.MODEL.EXT_INPUT_DIM

        if cfg_node.MODEL.EXT_INPUT_READIN_DIM > 0:
            self.ext_units = ext_units = cfg_node.MODEL.EXT_INPUT_READIN_DIM
            
        self.clip_value = cfg_node.MODEL.CELL_CLIP
    ...
    def build(self, input_shapes):
        ...
        # add the generator
        if self.cfg_node.MODEL.GEN_CELL_TYPE == 'gru':
        ...
        elif self.cfg_node.MODEL.GEN_CELL_TYPE == 'simple':
            self.gen_gru_cell = ClippedSimpleCell(
                self.gen_units,
                kernel_initializer=variance_scaling,
                recurrent_initializer=variance_scaling,
                #bias_initializer=ones_zeros(self.gen_units),
                recurrent_regularizer=DynamicL2(self.gen_l2_recurrent_weight),
                name='gen_simple_cell', # name will not reflect change to cell type
                clip_value=self.clip_value,
                train_h0=False,
            )
    ...
    def call(self, inputs, states, training=False):
        ...
        if self.cfg_node.MODEL.ZERO_EXT_INPUT:
            ext_input_step = tf.zeros_like(ext_input_step)
        ...
        if self.use_con:
            ...
            if self.cfg_node.MODEL.SAMPLE_POSTERIORS:
                ...
                # pass the mean in deterministic mode
                con_output = co_mean

            if self.cfg_node.MODEL.ZERO_CON_OUTPUT:
                con_output = tf.zeros_like(con_output)
