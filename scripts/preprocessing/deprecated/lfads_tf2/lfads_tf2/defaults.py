...
_C.MODEL.EXT_INPUT_READIN_DIM = -1 # The number of external inputs to be fed into the LFADS model.
_C.MODEL.READIN_DIM = -1 # The low-dim readin matrix for alignment compatibility (turn off with -1)
_C.MODEL.ENC_INPUT_DIM = _C.MODEL.DATA_DIM # The dimension of the input to the encoders. Overwritten when model is constructed depending on READIN_DIM
_C.MODEL.READIN_PCR_INIT = False # loads low-dim readin matrix from 'pcr_alignment.h5' file in data dir if True
_C.MODEL.READIN_FREEZE = False # freezes low-dim readin matrix during training
_C.MODEL.READOUT_FREEZE = False # freezes readout matrix from factors -> output params during training
...
_C.MODEL.GEN_DIM = 200 # The hidden dimension of the generator RNNCell.
_C.MODEL.FAC_DIM = 50 # The dimension of the learned factors.
_C.MODEL.GEN_CELL_TYPE = 'gru' # The type of RNN cell to use in the generator
...
_C.MODEL.RECON_TYPE = 'Poisson' # the type of reconstruction loss function to use; options: 'poisson', 'gaussian', 'mse'
_C.MODEL.SAMPLE_POSTERIORS = True # When false, passes the means of the posterior distributions.
_C.MODEL.ZERO_CON_OUTPUT = False # When true, zeros the controller output to remove the effect of inferred input on generator state. Useful during posterior sampling to examine zero-input dynamics after training
_C.MODEL.ZERO_EXT_INPUT = False # When true, zeros the controller output to remove the effect of inferred input on generator state. Useful during posterior sampling to examine zero-input dynamics after training
...
_C.TRAIN.DATA.LOG_TRANSFORM = False # whether to add a log transformation step to dataset pipeline
_C.TRAIN.DATA.AUGMENT = CN()
_C.TRAIN.DATA.AUGMENT.JITTER_WIDTH = 0 # half the width of a uniform distribution used to generate jitter
_C.TRAIN.DATA.AUGMENT.TYPE = 'Jitter' # half the width of a uniform distribution used to generate jitter
...
_C.TRAIN.REDUCE_MEAN = True # whether to use mean or sum to calculate recon cost for each sample
...
_C.TRAIN.L2.EXT_INPUT_READIN_SCALE = 5e-1 # the scale to weight the L1 of the ext input linear readin
