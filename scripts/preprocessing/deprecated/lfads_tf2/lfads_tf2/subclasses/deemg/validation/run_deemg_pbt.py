...
from tune_tf2.defaults import DEFAULT_CONFIG_DIR
from lfads_tf2.utils import flatten

# ---------- PBT I/O CONFIGURATION ----------
# the default configuration file for the LFADS model
CFG_PATH = '/snel/home/lwimala/Projects/future/lfads_tf2/lfads_tf2/subclasses/deemg/validation/deemg.yaml'
# the directory to save PBT runs (usually '~/ray_results')
PBT_HOME = path.expanduser('~/tmp/deemg_tf2_test_runs')
# the name of this PBT run (run will be stored at {PBT_HOME}/{PBT_NAME})
RUN_NAME = 'pbt_run_004' # the name of the PBT run
# the dataset to train the PBT model on
DATA_DIR = '/snel/home/lwimala/tmp/deemg_tf2_test_runs/run_007/deemg_input/'
DATA_PREFIX = 'deemg'
...
HYPERPARAM_SPACE = {
    'TRAIN.LR.INIT': HyperParam(1e-4, 5e-3, explore_wt=0.3, 
        enforce_limits=True, init=0.002),
    'MODEL.DROPOUT_RATE': HyperParam(0.0, 0.7, explore_wt=0.3,
    ...
    'TRAIN.L2.GEN_SCALE': HyperParam(1e-5, 1e-1, explore_wt=0.8),
    'TRAIN.L2.CON_SCALE': HyperParam(1e-5, 1e-1, explore_wt=0.8),
    'TRAIN.KL.CO_WEIGHT': HyperParam(1e-6, 1e-4, explore_wt=0.8),
    'TRAIN.KL.IC_WEIGHT': HyperParam(1e-6, 1e-4, explore_wt=0.8),
}
...
tuneLFADS = create_trainable_class(EPOCHS_PER_GENERATION, model_type=MODEL_TYPE)
...
model = deEMG(model_dir=best_model_dest)
