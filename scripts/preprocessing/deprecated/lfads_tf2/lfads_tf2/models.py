...
from lfads_tf2.layers import Encoder, Decoder, AutoregressiveMultivariateNormal, ModelReadin, ModelReadout, BaseMultiDatasetDense
from lfads_tf2.initializers import variance_scaling
from lfads_tf2.augmentations import log_transform_enc_input
from lfads_tf2.regularizers import DynamicL2
from lfads_tf2 import recon_losses, augmentations
...
class LFADS(Model):
    ...
    def get_input_shapes(self, batch_size=None):
        ...
        ds_ixs_shape = [batch_size,]
        return data_shape, ext_input_shape, ds_ixs_shape
    ...
    def build_model_from_init_call(self,
       ...
       # ===== CHECK SEQ_LEN LOGIC =====
        ...
        # ===== CREATE THE READIN LAYER =====
        if cfg.MODEL.READIN_DIM > 0:
            self.lgr.info('Creating readin matrices for data.')
            # pass input data through readin matrix
            if cfg.MODEL.READIN_PCR_INIT:
                self.lgr.info('Loading PCR alignment readin matrices from file.')
            else:
                self.lgr.info('Randomly initializing readin matrices.')
            self.readin = ModelReadin(cfg, self.ds_names, self.ds_dims, self.ds_idx,
                                     cfg.MODEL.DATA_DIM, cfg.MODEL.READIN_DIM,
                                     reg_scale=cfg.TRAIN.L2.READIN_SCALE,
                                     pre_init_from_file=cfg.MODEL.READIN_PCR_INIT
            )
            if self.cfg.MODEL.READIN_FREEZE:
                self.lgr.info('Freezing readin matrices.')
                self.readin.freeze_matrices()
            enc_input_dim = cfg.MODEL.READIN_DIM
        else:
            # directly input data to encoders
            enc_input_dim = cfg.MODEL.DATA_DIM

        # ===== CREATE THE EXTERNAL INPUT READIN LAYER ====
        if cfg.MODEL.EXT_INPUT_READIN_DIM > 0:
            self.lgr.info('Creating readin matrices for external inputs.')
            self.ext_input_readin = BaseMultiDatasetDense(
                cfg, self.ds_names, self.ds_dims, self.ds_idx,
                cfg.MODEL.EXT_INPUT_DIM, cfg.MODEL.EXT_INPUT_READIN_DIM,
            )
            
        # ===== CREATE THE ENCODER AND DECODER =====
        # set encoder input dim based on readin
        cfg.defrost()
        cfg.MODEL.ENC_INPUT_DIM = enc_input_dim
        cfg.freeze()
        ...
        if cfg.TRAIN.ENCODERS_ONLY:
            ...
            self.readout.trainable = False

        # ===== CREATE THE READOUT LAYER =====
        self.recon_loss = getattr(recon_losses, cfg.MODEL.RECON_TYPE)()
        self.lgr.info(f"Using {type(self.recon_loss).__name__} noise emissions.") 
        self.readout = ModelReadout(
            cfg, self.ds_names, self.ds_dims, self.ds_idx, \
            cfg.MODEL.FAC_DIM, cfg.MODEL.DATA_DIM, \
        )

        if self.cfg.MODEL.READOUT_FREEZE:
            self.lgr.warn('Freezing readout matrices.')
            self.readout.freeze_matrices()
        ...
        # set value to replace nans for selective backprop
        if type(self.recon_loss).__name__ == 'Gamma':
            self.sb_fill_val = 0.1 # strictly positive
        else:
            self.sb_fill_val = 0
        ...
        self.logging_metrics = [
            ...
            'recon_heldin', # negative log likelihood on seen training data
            'recon_heldout', # negative log likelihood on unseen training data
            'smth_recon_heldin', # smoothed negative log likelihood on seen training data
            'smth_recon_heldout', # smoothed negative log likelihood on unseen training data
            ...
            'val_recon_heldin', # negative log likelihood on seen validation data
            'val_recon_heldout', # negative log likelihood on unseen validation data
            'smth_val_recon_heldin', # smoothed negative log likelihood on seen validation data
            'smth_val_recon_heldout', # smoothed negative log likelihood on unseen validation data
    ...
    def build_graph(self):
        ...
        data_shape, ext_input_shape, ds_ixs_shape = self.get_input_shapes()
        # single step of training or validation
        self._graph_step = tf.function(
            func=self._step,
            input_signature=[
                BatchInput(
                    tf.TensorSpec(shape=data_shape), # data used for input
                    tf.TensorSpec(shape=data_shape), # data used for recon
                    tf.TensorSpec(shape=data_shape, dtype=tf.bool), # mask for sample validation
                    tf.TensorSpec(shape=data_shape, dtype=tf.bool), # mask for recon sample validation
                    tf.TensorSpec(shape=ext_input_shape), # external inputs
                    tf.TensorSpec(shape=ds_ixs_shape, dtype=tf.int64))] # dataset name
        )
        ...
        self.graph_call = tf.function(
            ...
            input_signature=[
                LFADSInput(
                    ...
                    tf.TensorSpec(shape=ds_ixs_shape, dtype=tf.int64)),
    ...
    def update_config(self, config):
        ...
        self.readout.trainable = not encoders_only
        if encoders_only ^ old_cfg.TRAIN.ENCODERS_ONLY:
            ...
            self.build_graph()
        # re-configure loggers if necessary
        if not node.TRAIN.TUNE_MODE:
            print('Updating TUNE_MODE to False. Setting console logger to INFO')
            self.lgr.handlers[0].setLevel(logging.INFO)
        else:
            print('Updating TUNE_MODE to True. Setting console logger to WARNING')
            self.lgr.handlers[0].setLevel(logging.WARNING)
        ...
        if old_cfg.MODEL.READIN_DIM > 0:
            self.readin.update_config(config, node.TRAIN.L2.READIN_SCALE)        
        if old_cfg.MODEL.EXT_INPUT_READIN_DIM > 0:
            self.ext_input_readin.update_config(config)        
        self.encoder.update_config(config)
        self.decoder.update_config(config)
        self.readout.update_config(config)        
    ...
    def call(self, lfads_input, sample_and_average=tf.constant(True)):
        ...
        lfads_input : lfads_tf2.tuples.LFADSInput
            ...
            external inputs.
        sample_and_average : bool, optional
            Whether to format inferred parameters for training or posterior
            sample and average (e.g. logrates are helpful for numerical 
        ...
        mcfg = self.cfg.MODEL
        # separate the inputs
        data, ext_input, dataset_id_ixs = tf.nest.flatten(lfads_input)
        if mcfg.READIN_DIM > 0:
            data = self.readin(data, dataset_id_ixs)
        ...
        if mcfg.EXT_INPUT_READIN_DIM > 0:
            ext_input = self.ext_input_readin(ext_input, dataset_id_ixs)
        # pass initial condition and controller input through decoder network
        dec_input = DecoderInput(ic_samp=ic, ci=ci, ext_input=ext_input)
        dec_output = self.decoder(dec_input, training=self.training)
        
        co_mean, co_stddev, factors, gen_states, \
            gen_init, gen_inputs, con_states = dec_output

        output_params = self.readout(factors, dataset_id_ixs)
        output_params = self.recon_loss.process_output_params(
            output_params, sample_and_average=sample_and_average)
    ...
    def load_datasets_from_file(self, data_dir, prefix):
        ...
        for ii, fname in enumerate(filenames):
            train_d = data[ii][0]
            valid_d = data[ii][1]
            if len(data[ii][0].shape) == 2:
                train_d = train_d[:,:,np.newaxis]
                valid_d = valid_d[:,:,np.newaxis]
            train_data[fname] = train_d
            valid_data[fname] = valid_d
            data_dims[fname] = train_d.shape[2]
            data_idx[fname] = ii

        #train_data = {fname:data[ii][0] for ii, fname in enumerate(filenames)}
        #valid_data = {fname:data[ii][1] for ii, fname in enumerate(filenames)}
        #data_dims = {fname:data[ii][0].shape[2] for ii, fname in enumerate(filenames)}
        #data_idx = {fname:ii for ii, fname in enumerate(filenames)}

        # save the dataset names as an attribute of the model, list 
        self.ds_names = filenames
        # save the dataset dims as an attribute of the model, dict indexed using ds_names
        self.ds_dims = data_dims
        # save an integer index for each dataset tha, dict indexed using ds_names
        self.ds_idx = data_idx

        # log dataset names
        self.lgr.info(f"Found {len(self.ds_names)} datasets")                
        for i, ds_name in enumerate(self.ds_names):
            self.lgr.info(f"== Dataset #{i}: {ds_name} ({self.ds_dims[ds_name]} channels)")        
        
        # find max input size
        max_data_dim = max(data_dims.values())

        # set data dim to max input dim
        self.cfg.defrost()
        self.cfg.MODEL.DATA_DIM = max_data_dim
        self.cfg.freeze()

        self.lgr.info(f"Max input dim: {max_data_dim} channels")
        # pad the input data
        for ii, fname in enumerate(filenames):
            ds_dim = train_data[fname].shape[2]
            dim_diff = max_data_dim - ds_dim
            if dim_diff > 0:
                self.lgr.warn(f"Padding Dataset #{ii} with {dim_diff} channels to match max input dim")                                        
                # pad channels dim to match max dim
                train_data[fname] = np.pad(train_data[fname],
                                           ((0,0),(0,0),(0,dim_diff)),
                                           constant_values=(0))
                valid_data[fname] = np.pad(valid_data[fname],
                                           ((0,0),(0,0),(0,dim_diff)),
                                           constant_values=(0))

        # Load any external inputs
        if self.cfg.MODEL.EXT_INPUT_DIM > 0:
            self.lgr.info(f"Loading external inputs for training")
            ext_inputs, filenames = load_data(
                data_dir, prefix=prefix, signal='ext_input', with_filenames=True)
            train_ext = {fname:ext_inputs[ii][0] for ii, fname in enumerate(filenames)}
            valid_ext = {fname:ext_inputs[ii][1] for ii, fname in enumerate(filenames)}
        else:
        ...
        try:
            ...
            train_inds = {fname:inds[ii][0] for ii, fname in enumerate(filenames)}
            valid_inds = {fname:inds[ii][1] for ii, fname in enumerate(filenames)}            
    ...
    def load_datasets_from_arrays(self, loadable_data):
        ...
        self.train_tuple = {
            k : BatchInput(
                data=train_data[k],
                recon_data=train_data[k],
                sv_mask=train_sv_mask[k],
                recon_sv_mask=train_sv_mask[k],
                ext_input=train_ext[k],
                dataset_id_ixs=np.array([self.ds_idx[k]]*train_data[k].shape[0])) # all fields must have same first dimension
                #dataset_id_ixs=np.array([k]*train_data[k].shape[0])) # all fields must have same first dimension
            for k in data_keys
        }
        self.valid_tuple = {
            k : BatchInput(
                data=valid_data[k],
                recon_data=valid_data[k],
                sv_mask=valid_sv_mask[k],
                recon_sv_mask=valid_sv_mask[k],
                ext_input=valid_ext[k],
                dataset_id_ixs=np.array([self.ds_idx[k]]*valid_data[k].shape[0])) # all fields must have same first dimension
                #dataset_id_ixs=np.array([k]*valid_data[k].shape[0])) # all fields must have same first dimension
            for k in data_keys
        }
        ...
        self.all_train_tuples = BatchInput(
            ...
            recon_data=np.vstack([self.train_tuple[k].recon_data for k in data_keys]),
            sv_mask=np.vstack([self.train_tuple[k].sv_mask for k in data_keys]),
            recon_sv_mask=np.vstack([self.train_tuple[k].recon_sv_mask for k in data_keys]),
            ext_input=np.vstack([self.train_tuple[k].ext_input for k in data_keys]),
            dataset_id_ixs=np.concatenate([self.train_tuple[k].dataset_id_ixs for k in data_keys])
        )
        ...
        self.all_valid_tuples = BatchInput(
            ...
            recon_data=np.vstack([self.valid_tuple[k].recon_data for k in data_keys]),
            sv_mask=np.vstack([self.valid_tuple[k].sv_mask for k in data_keys]),
            recon_sv_mask=np.vstack([self.valid_tuple[k].recon_sv_mask for k in data_keys]),
            ext_input=np.vstack([self.valid_tuple[k].ext_input for k in data_keys]),
            dataset_id_ixs=np.concatenate([self.valid_tuple[k].dataset_id_ixs for k in data_keys])
        )
        ...
        if self.cfg.TRAIN.BATCH_SIZE > len(self.all_train_tuples.data):
    ...
    def add_sample_validation(self, model_call):
        ...
        samples and the heldout samples separately. `recon_heldin` is 
        ...
        def sv_step(batch):
            # unpack the batch
            data, recon_data, sv_mask, recon_sv_mask, *_ = batch
            heldin_mask = sv_mask
            heldout_mask = tf.logical_not(recon_sv_mask)
            ...
            new_batch = BatchInput(
                data=heldin_data,
                recon_data=heldin_data,
                sv_mask=batch.sv_mask,
                recon_sv_mask=batch.recon_sv_mask,
                ext_input=batch.ext_input,
                dataset_id_ixs=batch.dataset_id_ixs)
            output_params, posterior_params, loss_mask = model_call(new_batch)
            # compute the recon cost of the observed samples
            #recon_heldin = self.recon_cost(maskable_data, output_params, wt_mask)
            if self.sv_keep < 1:
                maskable_data = recon_data[:, ic_enc_seq_len:, :]
                ...
                recon_heldout = self.recon_cost(maskable_data, output_params, wt_heldout_mask)
            else:
                recon_heldout = np.nan
            self._update_metrics({
                'recon_heldout': recon_heldout,
            }, batch_size=data.shape[0])
            # mask the loss for heldout data elements (use reconstructon data to build mask)
            wt_heldin_mask = tf.cast(recon_sv_mask, tf.float32) / self.sv_keep
            loss_mask = wt_heldin_mask * loss_mask

            return output_params, posterior_params, loss_mask
            #return recon_heldin, recon_heldout, posterior_params
    ...
    def add_coordinated_dropout(self, model_call):
        ...
        def cd_step(batch):
            ...
            new_batch = BatchInput(
                data=cd_input,
                recon_data=cd_input, # dummy 
                sv_mask=batch.sv_mask,
                recon_sv_mask=batch.sv_mask, # dummy 
                ext_input=batch.ext_input,
                dataset_id_ixs=batch.dataset_id_ixs)
            output_params, posterior_params, loss_mask = model_call(new_batch)
            # block the gradients with respect to the masked outputs
            rep_cnst = tf.constant([1,1,1,self.recon_loss.n_params], tf.int32)
            grad_mask = tf.tile(tf.expand_dims(grad_mask,-1), rep_cnst)
            output_params = block_gradients(output_params, grad_mask)
            return output_params, posterior_params, loss_mask
    ...
    def add_selective_backprop(self, model_call):
        ...
        def sb_step(batch):
            ...
            new_batch = BatchInput(
                data=sb_masked_data,
                recon_data=sb_masked_data, #dummy
                sv_mask=batch.sv_mask,
                recon_sv_mask=batch.sv_mask, #dummy
                ext_input=batch.ext_input,
                dataset_id_ixs=batch.dataset_id_ixs)
            ...
            recon_nan_mask = tf.math.is_nan(recon_data)
            loss_mask = tf.cast(~recon_nan_mask, tf.float32) * loss_mask
            # scale loss
            #calc_samples = tf.reduce_sum(loss_mask, axis=(1,2))
            #scale_factor = tf.reduce_min(calc_samples)/calc_samples
            #scale_factor = calc_samples/tf.reduce_max(calc_samples)
            #loss_mask = tf.math.multiply(
            #    1/scale_factor[:,tf.newaxis, tf.newaxis],
            #    loss_mask
            #)
            return output_params, posterior_params, loss_mask
    ...
    def batch_call(self, batch):
        ...
        lfads_input = LFADSInput(data=batch.data,
                                 ext_input=batch.ext_input,
                                 dataset_id_ixs=batch.dataset_id_ixs)
        if self.cfg.TRAIN.EAGER_MODE: 
            output = self.call(lfads_input, sample_and_average=False)
        else:
            output = self.graph_call(lfads_input, sample_and_average=False)
        ...
        loss_mask = tf.ones_like(batch.data)
        return output_params, posterior_params, loss_mask

    def recon_cost(self, data, output_params, wt_mask=None):
        """Computes the reconstruction cost of the data, given 
        predicted output parameters. The actual loss selected depends on 
        the cfg.MODEL.LOSS_TYPE hyperparameter.

        Parameters
        ----------
        data : tf.Tensor
            A BxTxN tensor of data.
        output_params : tf.Tensor
            A BxTxNxN_PARAMS tensor of parameters that describe the data 
            distribution.
        wt_mask : tf.Tensor
        ...
        -------
        tf.Tensor
            A scalar tensor representing the reconstruction cost for 
            the batch.        
        
        """
        if wt_mask is None:
            wt_mask = tf.ones_like(data)
        # Compute the reconstruction cost for each data point
        recon_all = self.recon_loss.compute_loss(data, output_params)
        # Set masked losses to zero and scale up others appropriately
        recon_masked = recon_all * wt_mask
        if self.cfg.TRAIN.REDUCE_MEAN:
            # Average over all elements of the data tensor
            recon = tf.reduce_mean(recon_masked)
        else:
            # Sum over inner dimensions, average over batch dimension
            recon = tf.reduce_mean(tf.reduce_sum(recon_masked, axis=[1,2]))
        return recon
    ...
    def weighted_l2_loss(self):
        ...
        if self.cfg.MODEL.READIN_DIM > 0:
            readin_size = self.cfg.MODEL.READIN_DIM * self.cfg.MODEL.DATA_DIM
            l2 += tf.reduce_sum(self.readin.losses) / readin_size
        if self.cfg.MODEL.EXT_INPUT_READIN_DIM > 0:
            readin_size = self.cfg.MODEL.EXT_INPUT_READIN_DIM * self.cfg.MODEL.EXT_INPUT_DIM
            l2 += tf.reduce_sum(self.ext_input_readin.losses) / readin_size
    ...
    def _step(self, batch):
        ...
        ic_enc_seq_len = self.cfg.MODEL.IC_ENC_SEQ_LEN
        input_data = batch.data[:, ic_enc_seq_len:, :]
        recon_data = batch.recon_data[:, ic_enc_seq_len:, :]
        nan_data = tf.math.is_nan(input_data)
        nan_recon_data = tf.math.is_nan(recon_data)
        input_data = tf.where(nan_data, tf.zeros_like(input_data), input_data)

        # for reconstruction data we need to be 
        recon_data = tf.where(nan_recon_data,
                              tf.ones_like(recon_data)*self.sb_fill_val,
                              recon_data)
        ...
        if self.training:
            with tf.GradientTape() as tape:
                ...
                output_params, posterior_params, loss_mask = self.train_call(batch)
                recon_heldin = self.recon_cost(recon_data, output_params, loss_mask)
                l2 = self.weighted_l2_loss()
                kl = self.weighted_kl_loss(*posterior_params)

                loss = recon_heldin + self.l2_ramping_weight * l2 \
        ...
        # ----- VALIDATION STEP -----
            ...
            output_params, posterior_params, loss_mask = self.val_call(batch)
            recon_heldin = self.recon_cost(recon_data, output_params, loss_mask)
            ...
            self._update_metrics({
                'val_loss': loss,
                'val_recon_heldin': recon_heldin,
                'val_recon_heldout': np.nan,
                'val_wt_kl': kl,
                'val_wt_l2': l2,
            }, batch_size=batch.data.shape[0])
    ...
    def build_dataset_pipelines(self, drop_remainder=False):
        ...
        aug_cfg_node = self.cfg.TRAIN.DATA.AUGMENT
        aug_type = aug_cfg_node.TYPE
        self.lgr.info(f"Applying {aug_type} data augmentation during training")
        aug = getattr(augmentations, aug_type)()
        augment = lambda batch : aug.augment_func(batch, aug_cfg_node)
        train_dataset = (
                    self._train_ds
        ...
        if self.cfg.TRAIN.DATA.LOG_TRANSFORM:
            self.lgr.info(f"Log transforming input to encoders")
            transform_enc_input = lambda batch : log_transform_enc_input(batch)
            train_dataset = train_dataset.map(transform_enc_input,
                                              num_parallel_calls=AUTOTUNE)
            valid_dataset = valid_dataset.map(transform_enc_input,
                                              num_parallel_calls=AUTOTUNE)
        ...
        # fix: dont drop remainder on validation set
        valid_dataset = (valid_dataset
                         # divide into batches
                         .batch(self.cfg.TRAIN.BATCH_SIZE, drop_remainder=False))
    ...
    def train_epoch(self, loadable_data=None):
        ...
        self._update_metrics({
            'smth_recon_heldin': smth_metric('recon_heldin'),
            'smth_recon_heldout': smth_metric('recon_heldout'),
            'smth_val_recon_heldin': smth_metric('val_recon_heldin'),
            'smth_val_recon_heldout': smth_metric('val_recon_heldout'),
        })
        ...
        train_template = ' - '.join([
            "    loss: {loss:.3f}",
            "    recon_heldin: {recon_heldin:.3f}",
            "    recon_heldout: {recon_heldout:.3f}",
            "    wt_kl: {wt_kl:.2E}",
            "    wt_l2: {wt_l2:.2E}",
            "gnorm: {gnorm:.2E}",
        ])
        val_template = ' - '.join([
            ...
            "val_recon_heldin: {val_recon_heldin:.3f}",
            "val_recon_heldout: {val_recon_heldout:.3f}",
        ...
        def check_earlystop(train_df):
            ...
            if cur_epoch > self.last_ramp_epoch:
                ...
                best_epoch = postramp_df.smth_val_recon_heldin.idxmin()
            ...
            if self.cur_patience.numpy() >= cfg.PATIENCE:
                self.lgr.info(f"STOPPING: No improvement in `smth_val_recon_heldin` for {cfg.PATIENCE} epochs.")                
                pass_check = False
            return pass_check
        ...
        if self.cfg.TRAIN.USE_TB:
            ...
            def make_figure(data, out_params, co_post):
                ...
                plt.subplot(2, 2, 4, title='All Output params')
                plt.imshow(tf.transpose(tf.squeeze(out_params[0,:,:,0])))
                return figure
            ...
            with self.summary_writer.as_default():
                ...
                with tf.name_scope('lfads'):
                    ...
                    if cur_epoch % 5 == 0:
                        ...
                        train_input = LFADSInput(data=batch.data,
                                                 ext_input=batch.ext_input,
                                                 dataset_id_ixs=batch.dataset_id_ixs)
                        val_input = LFADSInput(data=val_batch.data,
                                               ext_input=val_batch.ext_input,
                                               dataset_id_ixs=val_batch.dataset_id_ixs)
                        ...
                        t_out_params = t_output.output_params
                        posterior_params = (
                            t_output.ic_means,
                            t_output.ic_stddevs,
                            t_output.co_means,
                        ...
                        v_out_params = v_output.output_params
                        posterior_params = (
                            v_output.ic_means,
                            v_output.ic_stddevs,
                            v_output.co_means,
                        ...
                        train_fig = make_figure(self.all_train_tuples.data, t_out_params, t_co_post)
                        tf.summary.image('train_ouput', figure_to_tf(train_fig))
                        val_fig = make_figure(self.all_valid_tuples.data, v_out_params, v_co_post)
    ...
    def posterior_sample_and_average_call(self, lfads_input, n_samples):
        ...
        data, ext_input, dataset_id_ixs = lfads_input
        ...
        output_samples = [split_samp_and_batch(t, len(data)) \
        ...
        if not self.cfg.TRAIN.TUNE_MODE:
            ...
            co_means, co_stddevs, factors, gen_states, gen_init, gen_inputs, \
                con_states, ic_post_mean, ic_post_logvar = all_outputs
            
            # compute output parameters, where innermost dimension is param number
            output_params = self.output_linear(factors)
            output_params = self.recon_loss.process_output_params(
                output_params, sample_and_average=True)
    ...
    def sample_and_average(self,
        ...
        valid_dataset = self._valid_ds
        if self.cfg.TRAIN.DATA.LOG_TRANSFORM:
            AUTOTUNE = tf.data.experimental.AUTOTUNE
            transform_enc_input = lambda batch : log_transform_enc_input(batch)
            train_dataset = train_dataset.map(transform_enc_input, num_parallel_calls=AUTOTUNE)
            valid_dataset = valid_dataset.map(transform_enc_input, num_parallel_calls=AUTOTUNE)
        # perform sampling on both training and validation data
        for prefix, dataset in zip(['train_', 'valid_'], [train_dataset, valid_dataset]):
            data_len = len(self.all_train_tuples.data) if prefix == 'train_' else len(self.all_valid_tuples.data)
            inds = self.train_inds if prefix == 'train_' else self.valid_inds
            inds = np.concatenate(list(inds.values()))[:,np.newaxis]
            ...
            for batch in dataset.batch(batch_size):
                ...
                # unpack the batch
                data, _, _, _, ext_input, dataset_id_ixs = batch
                ...
                ds_id_ixs_all.append(dataset_id_ixs)
            ...
            ds_id_ixs_all = np.concatenate(ds_id_ixs_all)
            # compute output parameters, where innermost dimension is param number
            output_params = self.readout(factors, ds_id_ixs_all)
            output_params = self.recon_loss.process_output_params(
                output_params, sample_and_average=True)
            ...
            split_out = {self.ds_names[k] : [] for k in np.unique(ds_id_ixs_all)} # each list will hold divided output
            ...
            for field in samp_out._fields: 
                ...
                if tf.is_tensor(data):
                    # if a tensor
                    data = data.numpy()
                for k in np.unique(ds_id_ixs_all):
                    ds_name = self.ds_names[k]
                    ds_dim = self.ds_dims[ds_name]
                    where_dataset = np.where(ds_id_ixs_all == k)[0]
                    if len(data.shape) == 4:
                        split_out[ds_name].append(data[where_dataset, :, :ds_dim, :])
                    if len(data.shape) == 3:
                        split_out[ds_name].append(data[where_dataset, :, :])
                    elif len(data.shape) == 2:
                        split_out[ds_name].append(data[where_dataset, :])
                    elif len(data.shape) == 1:
                        # ic priors 
                        split_out[ds_name].append(data)
                            
            output_ds_names = split_out.keys()
            split_out = [SamplingOutput._make(split_out[k]) for k in output_ds_names]
            # writes the output to the a file in the model directory
            with h5py.File(output_file, 'a') as hf:
                for ii, k in enumerate(output_ds_names):
                    group = hf.create_group(prefix+k)
                    curr_out = split_out[ii]
                    output_fields = list(samp_out._fields)
                    for field in output_fields:
                        data = getattr(curr_out, field)
                        if field == 'inds':
                            # ensure we flatten inds to vector
                            data = data.squeeze()
                        else:
    ...
    def restore_weights(self, lve=True):
        ...
        cfg = self.cfg
        data_shape, ext_input_shape, ds_ixs_shape = self.get_input_shapes(10)
        noise = LFADSInput(
            data=np.ones(shape=data_shape, dtype=np.float32),
            ext_input=np.ones(shape=ext_input_shape, dtype=np.float32),
            dataset_id_ixs=np.full(shape=ds_ixs_shape, fill_value=0)
        )
        self.call(noise)
        if lve:
