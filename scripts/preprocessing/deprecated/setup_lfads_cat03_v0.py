...
ARRAY_SELECT = lfads_dataset_cfg[0]['DATASET']['ARRAY_SELECT']
# -- paths
base_name = (
...
lfads_save_dir = f'/snel/share/share/tmp/scratch/pbechef/bilateral_cat/cat03/nwb_lfads/runs/{base_name}/datasets/'
...
#     # Construct the full path for the NWB file
#     ds_path = os.path.join(ds_base_dir, f"{ds_name}.nwb")# --- load dataset from NWB
# logger.info(f"Loading {ds_name} from NWB")
# dataset = NWBDataset(ds_path, split_heldout=False)

for ds_name in ds_names:
    pkl_filename = f"{ds_name}_preproc.pkl"
    pkl_path = os.path.join(cache_dir, pkl_filename)

    logger.info(f"Loading PKL {pkl_filename} for side {ARRAY_SELECT}")
    with open(pkl_path, 'rb') as f:
        dataset = pickle.load(f)

    # Update config name
    lfads_dataset_cfg[0]["DATASET"]["NAME"] = ds_name
    ...
    def generate_spk_keep_chan_mask(dataset, array_select, xcorr_threshold):
        ...
        spk_keep_mask = np.array([ARRAY_SELECT in side for side in side_neurons]) #make array, spk_keep_mask
    ...
    logging.info("Generating spk mask.")
    ...
    if TYPE == "emg":
        ...
        ds_name = f"lfads_" + NAME + "_" + TYPE + "_" + str(BIN_SIZE) + ".h5"
        yaml_name = "cfg_" + NAME + "_" + TYPE + "_" + str(BIN_SIZE) + ".yaml"
        INTERFACE_FILE = os.path.join(
            pkl_dir,
            ARRAY_SELECT + "_" + NAME + "_" + TYPE + "_" + str(BIN_SIZE) + "_interface.pkl",
        )
        ...
        INTERFACE_FILE = os.path.join(
            pkl_dir,
            "interface" + "_" + NAME + "_" + ARRAY_SELECT + "_" + TYPE + "_" + str(BIN_SIZE) + ".pkl",
        )
